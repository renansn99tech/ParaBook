...
function AdminAprovacoes({ onFilaAlterada, onNotificar, onNavegar }) {
  ...
  const revisarVersao = async (item) => {
    try {
      const { data } = await api.get(`/dashboard/publicacoes/${item.id}/revisao/`);
      const rotulos = { isbn: 'ISBN', edicao: 'Edição', titulo: 'Título', paginas: 'Páginas', categoria_id: 'Categoria', ano_publicacao: 'Ano de publicação' };
      const categoria = data.dados.categoria_id ? await api.get(`/biblioteca/categorias/${data.dados.categoria_id}/`) : null;
      const detalhes = { ...data.dados, categoria_id: categoria?.data?.nome || 'Não informada' };
      await swal.fire({ title: 'Versão enviada', text: Object.entries(detalhes).map(([campo, valor]) => `${rotulos[campo] || campo}: ${valor || 'Não informado'}`).join(' · '), confirmButtonText: 'Conferir PDF', showCancelButton: true, cancelButtonText: 'Fechar' }).then(async (resposta) => {
        if (!resposta.isConfirmed || !data.pdf_disponivel) return;
        const arquivo = await api.get(`/dashboard/publicacoes/${item.id}/revisao/`, { params: { arquivo: 'pdf' }, responseType: 'blob' });
        const url = URL.createObjectURL(new Blob([arquivo.data], { type: 'application/pdf' }));
        try { await swal.fire({ title: 'PDF em revisão', html: `<iframe title="PDF em revisão" src="${url}" style="width:100%;height:60vh;border:0"></iframe>`, width: '90%', confirmButtonText: 'Fechar prévia' }); }
        finally { URL.revokeObjectURL(url); }
      });
    } catch { swal.fire({ icon: 'error', title: 'Não foi possível carregar a versão', text: 'Atualize a fila e tente novamente.' }); }
  };
  ...
  const decidir = async (item, acao) => {
    ...
    remover(item);
    try {
      await api.post(`/dashboard/moderacao/${item.categoria}/${item.id}/`, { acao, observacao, tentativa_id: item.tentativa_id });
