...
function AdminDenuncias({ onFilaAlterada, onNotificar, onNavegar }) {
  ...
  const decidir = async (item, acao) => {
    ...
    const confirmacao = await swal.fire({ title: acolher ? 'Acolher denúncia?' : 'Arquivar denúncia?', text: acolher ? 'O conteúdo será restringido conforme a regra atual de moderação.' : 'A denúncia será tratada como não procedente e arquivada.', icon: 'warning', showCancelButton: true, confirmButtonText: acolher ? 'Acolher' : 'Arquivar', cancelButtonText: 'Cancelar', confirmButtonColor: acolher ? BOTAO.perigo : BOTAO.padrao, cancelButtonColor: BOTAO.neutro });
    if (!confirmacao.isConfirmed) return;
    setProcessando((atuais) => [...atuais, chave]);
    remover(item);
    try {
      await api.post(`/dashboard/moderacao/${item.categoria}/${item.id}/`, { acao });
      onNotificar?.(acolher ? 'Denúncia acolhida e conteúdo restringido.' : 'Denúncia arquivada.');
  ...
  return (
    <section className="secao dash-fila-page">
      <h1>Denúncias Recebidas</h1><p className="admin-subtitulo">Analise conteúdo sinalizado usando o tom de alerta reservado à moderação.</p>
