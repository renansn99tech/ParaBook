...
function ConteudoComunidade() {
  ...
  const podeConfigurar = !comunidade?.demonstrativo && (admin || criador);
  ...
  const podePublicar = !comunidade?.demonstrativo && (membro || admin);
  ...
  return <main id="topo" className="pagina-comunidade" ref={paginaRef}>
    ...
    <section className="cc-banner" data-revelar>
      ...
      <div className="cc-banner-conteudo"><div className="cc-banner-titulo"><h1>{comunidade.nome}</h1>{comunidade.demonstrativo ? <span className="cc-status cc-status--oficial">Demonstrativo</span> : comunidade.em_manutencao ? <span className="cc-status cc-status--desativada"><i className="fa-solid fa-power-off" aria-hidden="true"></i>Desativada</span> : comunidade.criada_por_sistema ? <span className="cc-status cc-status--oficial"><i className="fa-solid fa-shield-halved" aria-hidden="true"></i>Oficial</span> : null}</div><p>{comunidade.descricao}</p><div className="cc-meta"><span><i className="fa-solid fa-user-group" aria-hidden="true"></i>{totalMembros} {totalMembros === 1 ? 'membro' : 'membros'}</span><span><i className="fa-regular fa-message" aria-hidden="true"></i>{postagens.length} {postagens.length === 1 ? 'postagem' : 'postagens'}</span><span><i className="fa-regular fa-calendar" aria-hidden="true"></i>{formatarMesAno(comunidade.data_criacao)}</span></div></div>
    ...
    <aside className="cc-aside" aria-label="Informações da comunidade">
      <section className="cc-side-card" data-revelar><header><span className="cc-side-icone"><i className="fa-solid fa-circle-info" aria-hidden="true"></i></span><h2>Sobre</h2></header><p>{comunidade.descricao}</p><div className="cc-capacidade"><div><span>Ocupação</span><strong>{comunidade.demonstrativo ? 'Somente visualização' : `${totalMembros}/${capacidade || '∞'}`}</strong></div>{!comunidade.demonstrativo && capacidade > 0 && <div className="cc-progresso" role="progressbar" aria-label={`Ocupação de ${comunidade.nome}`} aria-valuemin="0" aria-valuemax={capacidade} aria-valuenow={totalMembros}><span style={{ '--cc-ocupacao': `${ocupacao}%` }}></span></div>}</div><dl><div><dt>Criado por</dt><dd><CriadorComunidade comunidade={comunidade} /></dd></div><div><dt>Tipo</dt><dd>{comunidade.demonstrativo ? 'Demonstrativo' : comunidade.criada_por_sistema ? 'Oficial' : 'Da comunidade'}</dd></div><div><dt>Seu papel</dt><dd>{comunidade.demonstrativo ? 'Visitante' : papel}</dd></div></dl>{!admin && !comunidade.demonstrativo && !comunidade.em_manutencao && <button type="button" className={membro ? 'btn-ghost cc-participacao' : 'btn-primary cc-participacao'} onClick={handleAlternarParticipacao} disabled={alterandoParticipacao || (!membro && capacidade > 0 && totalMembros >= capacidade)}>{alterandoParticipacao ? <><i className="fa-solid fa-spinner fa-spin" aria-hidden="true"></i>Aguarde...</> : membro ? <><i className="fa-solid fa-arrow-right-from-bracket" aria-hidden="true"></i>Sair da comunidade</> : capacidade > 0 && totalMembros >= capacidade ? 'Comunidade lotada' : <><i className="fa-solid fa-user-plus" aria-hidden="true"></i>Entrar na comunidade</>}</button>}</section>
