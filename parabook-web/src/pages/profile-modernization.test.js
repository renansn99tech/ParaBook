...
test('informações oferece edição parcial, privacidade e drawers literários', () => {
  ...
  assert.match(profile, /api\.put\('\/auth\/idade\/'/);
  assert.match(profile, /chave_idempotencia: crypto\.randomUUID\(\)/);
  assert.match(profile, /exibir_aniversario_sem_ano:/);
  assert.match(profile, /exibir_email:/);
  assert.match(profile, /> Elegibilidade<\/dt>/);
  assert.match(profile, /Gerenciada de forma privada/);
  assert.match(profile, /Exibir apenas dia e mês do aniversário/);
  ...
  assert.match(perfilPublico, /pessoais\.exibir_aniversario_sem_ano/);
